from algebra import mod_inv, int_to_bytes
from random import randint

# Parameters for ElGamal
PARAM_P = 
PARAM_Q = 0x8CF83642A709A097B447997640129DA299B1A47D1EB3750BA308B0FE64F5FBD3
PARAM_G = 


# Key Generation
def EG_generate_keys():
    private_key = randint(1, PARAM_Q - 1)  # Secret key
    public_key = pow(PARAM_G, private_key, PARAM_P)  # g^x mod p
    return private_key, public_key


# Multiplicative Encryption
def EGM_encrypt(message, public_key):
    k = randint(1, PARAM_Q - 1)  # Random nonce
    r = pow(PARAM_G, k, PARAM_P)  # r = g^k mod p
    c = (message * pow(public_key, k, PARAM_P)) % PARAM_P  # c = m * y^k mod p
    return r, c


# Additive Encryption
def EGA_encrypt(message, public_key):
    k = randint(1, PARAM_Q - 1)  # Random nonce
    r = pow(PARAM_G, k, PARAM_P)  # r = g^k mod p
    c = (message + pow(public_key, k, PARAM_P)) % PARAM_P  # c = m + y^k mod p
    return r, c


# Decryption (for both versions)
def EG_decrypt(r, c, private_key, mode="multiplicative"):
    s = pow(r, private_key, PARAM_P)  # Shared secret s = r^x mod p
    if mode == "multiplicative":
        message = (c * mod_inv(s, PARAM_P)) % PARAM_P  # m = c * s^-1 mod p
    elif mode == "additive":
        message = (c - s) % PARAM_P  # m = c - s mod p
    else:
        raise ValueError("Invalid mode. Use 'multiplicative' or 'additive'.")
    return message


# Demonstration
if __name__ == "__main__":
    # Key generation
    private_key, public_key = EG_generate_keys()
    print("Private Key:", private_key)
    print("Public Key:", public_key)

    # Multiplicative encryption
    message = 123456789
    r, c = EGM_encrypt(message, public_key)
    print("Encrypted (r, c):", (r, c))
    decrypted_message = EG_decrypt(r, c, private_key, mode="multiplicative")
    print("Decrypted Message:", decrypted_message)

    # Additive encryption
    message = 987654321
    r, c = EGA_encrypt(message, public_key)
    print("Encrypted (r, c):", (r, c))
    decrypted_message = EG_decrypt(r, c, private_key, mode="additive")
    print("Decrypted Message:", decrypted_message)
